...
export default class NRDoc {
  ...
  async replaceContent(
    ...
    console.log('当前设置:', {
      useEmbeddingType: this.settings.useEmbeddingType,
      transcludeByDefault: this.settings.transcludeByDefault,
      noteLinkTemplate: this.settings.noteLinkTemplate
    }) // 添加调试信息

    if (this.settings.noteLinkTemplate !== undefined && this.settings.noteLinkTemplate !== '') {
      const link = await this.fileManager.generateMarkdownLink(currentNote, '', '', '')
      const newNoteLink = this.settings.useEmbeddingType ? `![[${fileName}]]` : `[[${fileName}]]`
      console.log('使用模板时生成的链接:', newNoteLink) // 添加调试信息
      ...
      const prefix = this.settings.useEmbeddingType || this.settings.transcludeByDefault ? '!' : ''
      contentToInsert = `${prefix}[[${fileName}]]`
      console.log('未使用模板时生成的链接:', contentToInsert) // 添加调试信息
    }
  ...
  async markdownLink(filePath: string): Promise<string> {
    ...
    console.log('生成的 Markdown 链接:', link) // 添加调试信息
    return this.settings.useEmbeddingType ? `!${link}` : link
  }
